...
/**
 * 
 * This class provides a fairly efficient implementation of a FIFO queue. We use
 * an array to implement the queue which is automatically increased in size when
 * necessary. Note that the queue is never shrunk in the current implementation.
 * 
 * @author Mark Astley
...
public class Queue {
	/**
	 * This constant sets the initial size of a queue when the default constructor
	 * is invoked to create a queue. Users requiring a different initial size may
	 * invoke the appropriate constructor below.
	 */
	public static final int INITIAL_SIZE = 10;

	/**
	 * This array holds the queue. We use a dynamically growing circular array to
	 * represent the queue. Queue size is doubled when necessary which should make
	 ...
	 * respectively. Technically, <em>back</em> points to the first open spot in the
	 * queue rather than the last element.
	 */
	int front;
	int back;
	...
	/**
	 * This private method is used to double the size of the queue when more space
	 * is required (as a result of calling enqueue). Note that because the
	 * implementation is in terms of a circular array, we have to be careful when
	...
	/**
	 * Remove an object from the front of the queue. The returned object is removed
	 * from the queue.
	 * 
	 * @return A reference to the <b>Object</b> at the front of the queue. Returns
	 *         <em>null</em> if the queue is empty.
	 */
	...
	/**
	 * This function allows arbitrary queue elements to be removed. Elements which
	 * are equal (according to the <em>equal</em> function) to the argument are
	 * removed from the queue. Note that this version of the function only removes
	 * the FIRST element found. Use the <em>removeAll</em> function to remove ALL
	 * elements equal to the argument.
	 * 
	...
	synchronized public boolean remove(Object rem) {
		...
		// PRAGMA [debug,osl.util.Queue] Log.println("<Queue.remove> Searching for
		// element: " + rem);
		...
		if (i == back)
			...
			// PRAGMA [debug,osl.util.Queue] Log.println("<Queue.remove> Element not found,
			// returning");
			return false;
		// PRAGMA [debug,osl.util.Queue] } else {
		// PRAGMA [debug,osl.util.Queue] Log.println("<Queue.remove> Element found at
		// position: " + i);
		// PRAGMA [debug,osl.util.Queue] }
	...
	/**
	 * This function has the same behavior as <em>remove</em> except that ALL
	 * matching elements are removed from the queue.
	 * 
	 * @param <b>rem</b> The object to be removed from the queue.
